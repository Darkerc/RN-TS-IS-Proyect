...
export default (options: ModalAddUserProps) => {
    ...
    const GET_NOT_USERS = gql`
    query getNotPacientes($IdUsuarioNutricionista: Int!){
        getNotPacientes(IdUsuarioNutricionista:$IdUsuarioNutricionista){
            IdUsuario
            Nombre
            PrimerApellido
            UsuarioImg
    ...
    `;
    const { data, loading, error ,refetch } = useQuery(GET_NOT_USERS,{variables:{IdUsuarioNutricionista:parseInt(userId)}});
    const UsersList = () => {
